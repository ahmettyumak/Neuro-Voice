...
def clear_ai_memory():
    ...
    logger.info("🧹 AI Modelleri hafızadan silindi ve VRAM tamamen temizlendi! (Sentence Modülü)")


# =============================================================================
# 3. SABİTLER VE YAPAY ZEKA MODELLERİ
# =============================================================================
ONNX_MODEL_FILE = "model_sentence_isoforest.onnx"
SCALER_FILE = "model_sentence_scaler.pkl"
COLUMNS_FILE = "model_sentence_columns.pkl"
SAMPLE_RATE = 16000
...
def load_ai_models():
    ...
    if whisper_model is None:
        logger.info("🧠 Whisper GPU (Sentence Task) starting...")
        try:
...
def isolate_patient_audio(segments_list, y, sr):
    if len(segments_list) < 2:
        return segments_list, y, " ".join([w.word for s in segments_list for w in s.words]).strip(), "", 0, 0

    patient_segments, doctor_segments, doc_texts, pat_texts = [], [], [], []
    doc_phrases = ["word", "sentence", "first", "next", "two words", "both words", "put both", "order", "brief", "okay",
                   "good", "ready", "make a"]
    ...
    for seg in segments_list:
        ...
        is_doctor = False
        if any(p in text for p in doc_phrases):
            is_doctor = True
        elif "?" in text:
            is_doctor = True
        elif len(words) <= 2 and not any(w in text for w in ["i", "my", "me", "no", "yes", "can't", "don't"]):
            is_doctor = True
