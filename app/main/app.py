...
st.set_page_config(page_title="Neurovoice AI - English (Cookie)", page_icon="🧠", layout="wide")
...
with col_right:
    ...
    with st.container(border=True):
        ...
        st.button("🖼️ Cookie (Picture)", type="primary", use_container_width=True)
        ...
        if st.button("🧩 Sentence (Syntax)", type="secondary", use_container_width=True): st.switch_page("pages/sentence.py")
...
with col_left:
    ...
    with st.container(border=True):
        st.markdown("**Cookie Task**")
        st.caption("Instructions: Please look at the standardized 'Cookie Theft' picture. Describe everything you see happening in the image. Speak naturally and try to mention all the actions, people, and details. Target: Assesses spontaneous speech, semantic memory, and information density.")
        ...
        # UYARIYI ÇÖZDÜK: use_container_width=True yerine width="stretch" kullanıyoruz
        initiate = st.button("⚡ INITIATE ANALYSIS", type="primary", width="stretch")

        if initiate:
            if audio_file is None:
                st.error("⚠️ Please upload or record an audio file first.")
            else:
                # 1. Ses verilerini kaydet
                ...
                # 2. Hasta verilerini (Yaş ve Cinsiyet) kaydet
