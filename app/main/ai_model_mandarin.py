...
ONNX_MODEL_FILE = "app/main/model_XGBOOST_ALL.onnx"
SCALER_FILE = "app/main/model_XGBOOST_ALL_scaler.pkl"
COLUMNS_FILE = "app/main/model_XGBOOST_ALL_columns.pkl"
SAMPLE_RATE = 16000
...
def load_ai_models():
    ...
    if whisper_model is None: whisper_model = WhisperModel("large-v3", device="cuda", compute_type="float16")
...
def extract_mandarin_features(file_path):
    ...
    features["Sozcuk_Entropisi"] = sum([- (count / word_count) * math.log2(count / word_count) for count in word_counts.values()])

    pos_tags = pseg.cut(full_text)
    noun_count, pronoun_count, verb_count = 0, 0, 0
    for pair in pos_tags:
        if pair.flag.startswith('n'): noun_count += 1
        elif pair.flag.startswith('r'): pronoun_count += 1
        elif pair.flag.startswith('v'): verb_count += 1
    ...
    if len(words_data) > 0:
        for i in range(1, len(words_data)):
            ...
            elif 0.2 <= bosluk < 0.5: kisa_takilma_sayisi += 1
...
def analyze_mandarin_for_streamlit(audio_path):
    ...
    # ... (Model ONNX tahmin kodları yukarıda aynı kalıyor)

    prob_dict = pred_onx[1][0]
    if isinstance(prob_dict, dict):
        prob_mci = prob_dict.get(1, 0.0) * 100  # Artık prob_ad değil, prob_mci
        prob_hc = prob_dict.get(0, 1.0) * 100
    else:
        prob_mci = prob_dict[1] * 100
        prob_hc = prob_dict[0] * 100

    # 🚀 SADECE HC ve MCI (Gerçekçi ve Bilimsel 2 Sınıf)
    if prob_mci < 40.0:
    ...
    # SONUÇLARI DÖNDÜRMEDEN HEMEN ÖNCE HAFIZAYI TEMİZLE!
    clear_ai_memory()
    ...
    return {
        ...
        "prob_mci": round(prob_mci, 1),  # prob_ad yerine prob_mci döndürüyoruz
