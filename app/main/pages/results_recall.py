...
st.set_page_config(page_title="Recall Report", page_icon="📖", layout="wide")

if 'analysis_results_recall' not in st.session_state or st.session_state.analysis_results_recall is None:
    st.switch_page("pages/recall.py")

res = st.session_state.analysis_results_recall

if res.get("status") == "error":
    st.markdown("<br><br>", unsafe_allow_html=True)
    st.error(f"🛑 Analysis Rejected: {res.get('message')}")
    st.info("Tip: Ensure the patient is actively trying to recall the 'Anna Thompson' story.")
    if st.button("⬅️ Go Back to Recall Page", type="primary"):
    ...
    def create_recall_pdf(fig_gauge, fig_radar):
        ...
        try:
            ...
            pdf.cell(0, 15, "NEUROVOICE AI - MEMORY RECALL REPORT", ln=True, align="C")
            ...
            pdf.set_font("helvetica", "", 12)
            pdf.cell(0, 8, f"Memory Deficit Risk: %{res['prob_hasta']}", ln=1)
            pdf.cell(0, 8, f"Isolation Score: {res.get('isolation_score', 0)}", ln=1)

            pdf.ln(10)
            pdf.set_font("helvetica", "B", 14)
            pdf.cell(0, 10, "Recall Profile Map:", ln=True)
            pdf.image(radar_path, x=110, y=115, h=70)

            pdf.ln(5)
            pdf.set_font("helvetica", "", 11)
            pdf.cell(0, 7, f"- 'I forgot/don't know': {det['unutma_ifadeleri']} times", ln=True)
            pdf.cell(0, 7, f"- Memory Pauses: {det['duraksama']}", ln=True)
            pdf.cell(0, 7, f"- Pronoun Ratio (Anomia): {det['anomi']:.2f}", ln=True)
    ...
    st.markdown(f"""
        <div style='background: linear-gradient(90deg, #0B1117 0%, #1A222C 100%); padding: 20px; border-radius: 12px; border-left: 5px solid #14b8a6; margin-bottom: 30px; display: flex; justify-content: space-between; align-items: center;'>
            <div><h2 style='margin: 0; color: #F8FAFC;'>Neurovoice AI Clinical Report</h2><p style='margin: 0; color: #14b8a6; font-size: 1.1rem;'>Target: English (Story Recall)</p></div>
    ...
    with col_verdict:
        ...
        fig_gauge = go.Figure(go.Indicator(
            ...
            title={'text': "Memory Deficit Risk", 'font': {'size': 18}},
            gauge={
    ...
    with col_radar:
        st.markdown("<h4 style='color: #F8FAFC;'>🕸️ Memory Recall Profile Map</h4>", unsafe_allow_html=True)
        # Recall'a özel Radar Map: Unutma ifadeleri kötü puan!
        norm_words = min(det['toplam_kelime'] * 2, 100)
        norm_anomia = max(100 - (det['anomi'] * 80), 0)
        norm_pauses = max(100 - (det['duraksama'] * 5), 0)
        norm_memory = max(100 - (det['unutma_ifadeleri'] * 25), 0)  # 4 unutma kelimesi = 0 puan

        fig_radar = go.Figure(go.Scatterpolar(
            r=[norm_words, norm_anomia, norm_pauses, norm_memory],
            theta=['Narrative Flow', 'Noun Recall', 'Cognitive Pauses', 'Memory Confident'],
    ...
    with col_raw:
        st.markdown("#### 📊 Memory Biomarkers")
        with st.container(border=True):
            st.write(f"**🧠 'I forgot/don't know' used:** {det['unutma_ifadeleri']} times")
            st.write(f"**🔤 Pronoun/Noun Ratio:** {det['anomi']:.2f}")
            st.write(f"**📉 Isolation Score:** {res['isolation_score']:.3f}")

    with col_anom:
        st.markdown("#### ⚠️ Recall Issues")
        with st.container(border=True):
            st.write(f"**⏸️ Memory Pauses:** {det['duraksama']}")
            if det['hata_yerleri']:
    ...
    with b1:
        pdf_bytes = create_recall_pdf(fig_gauge, fig_radar)
        if pdf_bytes:
            st.download_button(label="📄 Download PDF Report", data=pdf_bytes, file_name=f"Recall_Report_{age}.pdf",
                               mime="application/pdf", use_container_width=True)

    with b2:
        if st.button("💡 View Clinical Recommendations", type="secondary", use_container_width=True):
            st.session_state.source_page = "recall"
            st.switch_page("pages/recommendations.py")

    with b3:
        if st.button("🔄 Start New Recall Analysis", type="primary", use_container_width=True):
            st.session_state.audio_bytes = None
            st.session_state.analysis_results_recall = None
            st.switch_page("pages/recall.py")