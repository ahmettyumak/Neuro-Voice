...
st.set_page_config(page_title="Neurovoice AI - English (Recall)", page_icon="🧠", layout="wide")
...
with col_right:
    ...
    with st.container(border=True):
        ...
        st.button("📖 Recall (Story)", type="primary", use_container_width=True) # Aktif Sayfa
...
with col_left:
    ...
    with st.container(border=True):
        st.markdown("**Recall Task**")
        st.caption("Instructions: Listen to a short story carefully. Once it finishes, retell the story in your own words, including as many specific details as you can remember. Target: Assesses narrative memory, auditory comprehension, and chronological sequencing.")
        ...
        if initiate:
            if audio_file is None:
                ...
                # RECALL yükleme ekranına gidiyor!
                st.switch_page("pages/loading_recall.py")