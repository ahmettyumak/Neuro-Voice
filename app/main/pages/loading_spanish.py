...
from ai_model_spanish import analyze_spanish_for_streamlit

st.set_page_config(page_title="Analyzing Spanish...", page_icon="🌍", layout="centered")
...
st.markdown(f"""
    <div class="title-container">
        ...
        <p class="subtitle">{t("Módulo de Análisis Español (XGBoost)", "Spanish Analysis Module (XGBoost)")}</p>
    </div>
...
def update_status(text_es, text_en, percent):
    status_text.markdown(f"<div class='status-text'>{t(text_es, text_en)}</div>", unsafe_allow_html=True)
    progress_bar.progress(percent)

update_status("📡 Extrayendo biomarcadores acústicos (MFCC)...", "📡 Extracting Acoustic Biomarkers (MFCC)...", 15)
...
update_status("🧬 Ejecutando el modelo Maestro XGBoost...", "🧬 Running Master XGBoost Model...", 45)
results = analyze_spanish_for_streamlit(tmp_path)

update_status("⚖️ Calculando probabilidades de Alzheimer...", "⚖️ Calculating Alzheimer's Probabilities...", 85)
time.sleep(0.5)
os.remove(tmp_path)

st.session_state.analysis_results_spanish = results
st.switch_page("pages/results_spanish.py")