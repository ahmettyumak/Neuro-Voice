...
from ai_model_korean import analyze_korean_for_streamlit

st.set_page_config(page_title="Analyzing Korean...", page_icon="🇰🇷", layout="centered")
...
st.markdown(f"""
    <div class="title-container">
        ...
        <p class="subtitle">{t("한국어 분석 모듈 (Spacy NLP)", "Korean Analysis Module (Spacy NLP)")}</p>
    </div>
...
def update_status(text_ko, text_en, percent):
    status_text.markdown(f"<div class='status-text'>{t(text_ko, text_en)}</div>", unsafe_allow_html=True)
    progress_bar.progress(percent)

update_status("📡 한국어 언어 특징 추출 중...", "📡 Extracting Spacy KoNLP Biomarkers...", 15)
...
update_status("🧬 XGBoost AI 모델 실행 중...", "🧬 Running Master XGBoost Model...", 45)
results = analyze_korean_for_streamlit(tmp_path)

update_status("⚖️ 인지 저하 (MCI) 확률 계산 중...", "⚖️ Calculating MCI Probabilities...", 85)
time.sleep(0.5)
os.remove(tmp_path)

st.session_state.analysis_results_korean = results
st.switch_page("pages/results_korean.py")