...
st.set_page_config(page_title="Neurovoice AI - English (Fluency)", page_icon="🧠", layout="wide")
...
with col_right:
    ...
    with st.container(border=True):
        ...
        st.button("⏱️ Fluency (Words)", type="primary", use_container_width=True) # Aktif Sayfa
...
with col_left:
    ...
    with st.container(border=True):
        st.markdown("**Fluency Task**")
        st.caption("Instructions: Name as many words as you can that belong to a specific category (e.g., 'Animals') or start with a specific letter (e.g., 'F', 'A', 'S') within 60 seconds.Target: Measures linguistic control, executive function, and word retrieval speed.")
        ...
        if initiate:
            if audio_file is None:
                ...
                # FLUENCY yükleme ekranına gidiyor!
                st.switch_page("pages/loading_fluency.py")