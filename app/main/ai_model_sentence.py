...
def clear_ai_memory():
    ...
    # 1. Modelleri RAM'den tamamen sil
    ...
    # 2. Python'a "Boşalan yerleri işletim sistemine geri ver" emrini ver
    gc.collect()

    # 3. Ekran kartı (VRAM) önbelleğini zorla temizle (PyTorch yüklüyse)
    try:
        import torch
...
def force_load_nvidia_dlls():
    ...
    for p in set(paths):
        ...
        if os.path.exists(cublas_path):
            try: os.add_dll_directory(cublas_path)
            except: pass
            os.environ["PATH"] = cublas_path + os.pathsep + os.environ.get("PATH", "")
        if os.path.exists(cudnn_path):
            try: os.add_dll_directory(cudnn_path)
            except: pass
...
ONNX_MODEL_FILE = "app/main/model_sentence_isoforest.onnx"
SCALER_FILE = "app/main/model_sentence_scaler.pkl"
COLUMNS_FILE = "app/main/model_sentence_columns.pkl"
SAMPLE_RATE = 16000
MAX_AUDIO_DURATION = 600

VALIDATION_KEYWORDS = [
    "sentence", "sentences", "word", "words", "make", "use", "put", "together",
    "repeat", "say", "first", "next", "both", "read", "complete", "finish",
    "child", "hospital", "winter", "chair", "doctor", "sit"
]
...
def load_ai_models():
    global whisper_model, nlp_model
    if whisper_model is None: whisper_model = WhisperModel("base", device="cuda", compute_type="float16")
    if nlp_model is None: nlp_model = spacy.load("en_core_web_sm")

def clean_word(word): return re.sub(r'[^\w\s]', '', word.lower().strip())

def isolate_patient_audio(segments_list, y, sr):
    if len(segments_list) < 2: return segments_list, y, " ".join([w.word for s in segments_list for w in s.words]).strip(), "", 0, 0
    patient_segments, doctor_segments, doc_texts, pat_texts = [], [], [], []
    doc_phrases = ["word", "sentence", "first", "next", "two words", "both words", "put both", "order", "brief", "okay", "good", "ready", "make a"]
    ...
    for seg in segments_list:
        ...
        is_doctor = False
        if any(p in text for p in doc_phrases): is_doctor = True
        elif "?" in text: is_doctor = True
        elif len(words) <= 2 and not any(w in text for w in ["i", "my", "me", "no", "yes", "can't", "don't"]): is_doctor = True
...
def extract_test_features(file_path):
    ...
    patient_segments, patient_y, patient_text, doctor_text, pat_dur, doc_dur = isolate_patient_audio(list(segments), y, sr)
    ...
    if word_count < 3 or speech_duration < 1.5:
    ...
    if (match_count + doc_match_count) < 1:
        return None, "INVALID TASK DETECTED: This is NOT a 'Sentence Construction/Repetition' task!"
...
def analyze_sentence_for_streamlit(audio_path):
    ...
    try:
        ...
        return {"status": "error", "message": f"Sentence Model files missing: {e}"}
    ...
    if disease_percent >= 60.0:
        teshis, risk_level = "HIGH RISK (Syntactic Deficit Detected)", "high"
    elif 40.0 <= disease_percent < 60.0:
        teshis, risk_level = "AT RISK (Mild Cognitive Impairment - MCI)", "medium"
    else:
        teshis, risk_level = "HEALTHY (Normal Sentence Construction)", "low"
